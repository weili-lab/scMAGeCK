...
int ProcessGroups(GROUP_STRUCT *groups, int groupNum, LIST_STRUCT *lists, int listNum, double minPercentile, double maxPercentile);
...
int ComputePermutation(GROUP_STRUCT *groups, int groupNum, double minPercentile, double maxPercentile, int numOfRandPass);
//An improved version by permuting genes from groups
int ComputePermutationbyNumSG(GROUP_STRUCT *groups, int groupNum, double minPercentile, double maxPercentile, int numOfRandPass);
...
int ComputeLoValue(double *percentiles,     //array of percentiles
   ...
   double maxPercentile,   //minimum and maximum percentile, computation stops when maximum percentile is reached
 ...
 * Main entry
 */
int RRA_main (vector<string> & argv) {
  int i,flag;
  GROUP_STRUCT *groups;
  int groupNum;
  LIST_STRUCT *lists;
  ...
  double minPercentile, maxPercentile;
  int rand_passnum;
  ...
  {
     if (argv[i-1] == "-i"){
       strcpy(inputFileName, argv[i].c_str());
     }
     if (argv[i-1] == "-o"){
       strcpy(outputFileName, argv[i].c_str());
     }
     if (argv[i-1]== "-p"){
       maxPercentile = atof(argv[i].c_str());
     }
     if (argv[i-1] == "-P"){
       minPercentile = atof(argv[i].c_str());
     }
     if (argv[i-1]== "--control"){
       UseControlSeq=true;
       // load control sequences
       loadControlSeq(argv[i].c_str());
     }
     if (argv[i-1]== "--permutation"){
       rand_passnum= atoi(argv[i].c_str());
	...
	Rcpp::Rcout<<"Skipping gene "<<sn<<" for permutation ..."<<endl;
     ...
     if (argv[i-1] == "--min-percentage-goodsgrna"){
       min_percentage_goodsgrna= atof(argv[i].c_str());
     }
     if (argv[i-1] == "--min-number-goodsgrna"){
       min_number_goodsgrna= atoi(argv[i].c_str());
     }
     if (argv[i-1] == "--max-sgrnapergene-permutation"){
       max_sgnum_permutation_by_group= atoi(argv[i].c_str());
     }
  }
  for (i=1;i<argc;i++)
  {
     if (argv[i] == "--no-permutation-by-group"){
  ...
  if(permutation_by_group){
    i=ComputePermutationbyNumSG(groups, groupNum, minPercentile, maxPercentile, rand_passnum*groupNum); 
  }else{
    i=ComputePermutation(groups, groupNum, minPercentile, maxPercentile, rand_passnum*groupNum); 
  }
  if (i<=0)
  {
...
int ProcessGroups(GROUP_STRUCT *groups, int groupNum, LIST_STRUCT *lists, int listNum, double minPercentile, double maxPercentile)
{
  ...
  for (i=0;i<groupNum;i++){
    ...
    if(isallone){
      ...
      ComputeLoValue_Prob(tmpF, validsgs, groups[i].loValue, minPercentile, maxPercentile, tmpProb,groups[i].goodsgrnas);
    }
...
int ComputeLoValue(double *percentiles,     //array of percentiles
  ...
  for(int i=0;i<num;i++) tmpArray[i]=percentiles[i];
  ...
  for (i=0;i<num;i++){
    ...
    if ((tmpArray[i]>maxPercentile)|| (tmpArray[i]<minPercentile)){
      if(i==0){}
      else{
        break;
      }
...
int ComputeLoValue_Prob(double *percentiles,     //array of percentiles
  ...
  double &loValue,         //pointer to the output lo-value
  double minPercentile,   
  double maxPercentile,   //min and maximum percentile, computation stops when maximum percentile is reached
  double *probValue,
  ...
  for (pid=1;pid<(1<<num);pid++)
  {
    ...
    for (i=0;i<num;i++)
    {
      ...
      if ((tmpArray[i]>maxPercentile || tmpArray[i]<minPercentile )&&(i>0))
      {
        break;
      }
      // Beta (a,b, 0, frac)
    ...
    if(sgrnanum<=max_sgnum_permutation_by_group){// to avoid permutations on genes with too many sgRNAs (take too much time)
      ...
      for (i=0;i<scanPass;i++){
        for (j=0;j<groupNum;j++){
          ...
          ComputeLoValue(tmpPercentile, validsgs,randLoValue[randLoValueNum],minPercentile, maxPercentile, tmp_int);
          randLoValueNum++;
        }// end for j
      }//end for i
  ...
  for (i=0;i<scanPass;i++){
    for (j=0;j<groupNum;j++){
      ...
      if(isallone){
        ...
        ComputeLoValue_Prob(tmpPercentile, validsgs,randLoValue[randLoValueNum],minPercentile, maxPercentile,tmpProb,tmp_int);
      }
      randLoValueNum++;
    }// end for j
  }//end for i
