...
get_rank_tables_from_rra <- function(rankexp, bc_dox_u, rrapath = NULL, pcutoff = 0.3, tmpprefix = paste("sample_", 
    runif(1, 1, 10000), sep = ""), negctrlgenelist = "NonTargetingControlGuideForHuman", more_rra = "", 
    negsel = TRUE, possel = TRUE, keeptmp = FALSE) {
    rankexp = rankexp[names(rankexp) %in% rownames(bc_dox_u) & !is.na(bc_dox_u[names(rankexp), "barcode"])]
    if (length(rankexp) < 3) {
        message("Error: cannot find enough cells.")
        return(NULL)
    }
